...
!---------------------------------------------------------------------
  ncdfstatus = nf_create( name, NF_CLOBBER, ncid )
...
!---------------------------------------------------
  ncdfstatus = nf_def_dim( ncid, 'nx', nx, nxdim )
  if ( ncdfstatus .ne. NF_NOERR ) call  handle_error()
  ncdfstatus = nf_def_dim( ncid, 'ny', ny, nydim )
  if ( ncdfstatus .ne. NF_NOERR ) call  handle_error()
...
!-----------------------
  if (myrank .eq. 0) then
    ...
    call MPI_Barrier(comm,ier)
    call MPI_Send( r, ncount, MPI_DOUBLE_PRECISION,              &
                       0, rtag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( t, ncount, MPI_DOUBLE_PRECISION,              &
                       0, ttag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( u, ncount, MPI_DOUBLE_PRECISION,              &
                       0, utag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( v, ncount, MPI_DOUBLE_PRECISION,              &
                       0, vtag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( w, ncount, MPI_DOUBLE_PRECISION,              &
                       0, wtag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( bx, ncount, MPI_DOUBLE_PRECISION,              &
                       0, bxtag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( by, ncount, MPI_DOUBLE_PRECISION,              &
                       0, bytag, comm, ier )

    call MPI_Barrier(comm,ier)
    call MPI_Send( bz, ncount, MPI_DOUBLE_PRECISION,              &
                       0, bztag, comm, ier )    
  ...
  do k=1,nlayer
    do j=1,ny
      ...
      if ( iprec .eq. 8 ) then
        ...
        ncdfstatus = nf_put_vara_real(ncid,varid,istarting,      &
                                      icount, rwk)
      end if
      if ( ncdfstatus .ne. NF_NOERR) call handle_error()
    end do
...
!
! If version<=2 then set Tayl and Psi to zero and then set version to 3 
! If version>2 then we read values from data file
!----------------------------------------------------------------------
...
!-----------------------------------------------------
    if (iproc .eq. 0) then
      do k=1, nlayer_size
        do j=1,ny
          do i=1,nx
            if (varname .eq. 'R') wk1(i,j,k) = r(i,j,k)
            if (varname .eq. 'T') wk1(i,j,k) = t(i,j,k)
            if (varname .eq. 'U') wk1(i,j,k) = u(i,j,k)
            if (varname .eq. 'V') wk1(i,j,k) = v(i,j,k)
            if (varname .eq. 'W') wk1(i,j,k) = w(i,j,k)
            if (varname .eq. 'Bx') wk1(i,j,k) = bx(i,j,k)
            if (varname .eq. 'By') wk1(i,j,k) = by(i,j,k)
            if (varname .eq. 'Bz') wk1(i,j,k) = bz(i,j,k)
          end do
        end do
      end do
    else
...
!---------------------------------------------------
! Otherwise carry out a blocking receive to fill wk1      
!---------------------------------------------------
      call MPI_Recv( wk1, ncount, MPI_DOUBLE_PRECISION, &
...
!------------------
    do k=1,nlayer_size
	  ...
	  if ( ncdfstatus .ne. NF_NOERR ) call handle_error()
          ...
          do i=1,nx
            rwk(i) = real(wk1(i,j,k))
          end do
          ncdfstatus = nf_put_vara_real(ncid, varid, istarting,   &
                                          icount, rwk )
	     if ( ncdfstatus .ne. NF_NOERR ) call handle_error()
	else
	  write(*,*) 'Bad precision specified for ',varname
	end if
      end do
    end do
  end do
