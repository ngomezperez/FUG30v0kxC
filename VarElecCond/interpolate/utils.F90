  ...
  do k=1,nlayer
    ...
    do j=1,2*ny
      do i=1,2*nx  
        ...
        bsqav = bsqav + bxnew(i,j,k)*bxnew(i,j,k) &
             + bynew(i,j,k)*bynew(i,j,k) + bznew(i,j,k)*bznew(i,j,k)
      end do
    end do
    iz = izstart + k - 1
...
!-----------------------------------------  
  ubar = urbar/rbar
  vbar = vrbar/rbar
  menergy = 0.25d0*f*menergy*xzn
  kenergy = 0.25d0*kenergy*xzn
...
!-------------------------------------
  do k=1,nlayer
    do j=1,2*ny
      do i=1,2*nx  
        unew(i,j,k) = unew(i,j,k) - ubar
        vnew(i,j,k) = vnew(i,j,k) - vbar
      end do
    end do
  end do
...
!------------------------------
  real(kind=dp), intent(in)    :: z(2*nx, 2*ny, nlayer )
